...
from database import add_course, get_semester_amp_courses
...
class Downloader(tk.Tk):
    ...
    def __init__(self):
        ...
        ### course list window ###
        self.courses_label = tk.Label(self, text='No Courses Loaded', justify='left')
        self.courses_label.grid(row=8, column=0, columnspan=4, pady = 10, padx=5)
        ...
        self.amp_service_radio = tk.Radiobutton(self, text='AMP', variable=self.service_type, value='amp').grid(row=7, column=0)
        self.cap_service_radio = tk.Radiobutton(self, text='CAP', variable=self.service_type, value='cap').grid(row=7, column=1)
    ...
    def add_course(self):
        ...
        if course_id is '' or course_semester is '' or course_folder_name is '':
            ...
            print(service_type)
            add_the_course = add_course(course_id, course_semester, course_folder_name, service_type)
            if add_the_course:
                self.feed_back_label["text"] = "Course Added"
            else:
                self.feed_back_label['text'] = "Course Already Exists"
    ...
    def get_current_course_list(self):
        current_courses_str = ''
        current_courses = get_semester_amp_courses(load_config()['semester'])
